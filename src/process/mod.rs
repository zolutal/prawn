...
impl Process {
    pub fn new<T: AsRef<str>>(argv: impl AsRef<[T]>, _cfg: &ProcessConfig)
                              -> Result<Process, Error> {
        let args = argv.as_ref().iter().map(
                   |x| String::from(x.as_ref())).collect::<Vec<_>>();
        ...
        let orig_personality = {
            if !enable_aslr {
                ...
                personality(orig | linux_personality::ADDR_NO_RANDOMIZE).expect(
                            "Failed to disable ASLR");
                Some(orig)
            } else{
                None
            }
        ...
        if !enable_aslr {
            if let Some(orig_personality) = orig_personality {
                personality(orig_personality).expect(
        ...
        Ok(Process { handle: Arc::new(Mutex::new(handle)),
                     buffer: Buffer::default(), io })
    }
...
impl tubes::Tube for Process {
    ...
    fn recv_raw(&mut self, _numb: usize, timeout: Duration)
        ...
        if let Ok(recv_result) = &received {
            ...
            if let Ok(exit_status) = handle.try_wait() {
                dbg!(exit_status);
